import {o_db_client, o_db_connection_info} from "./o_db_client.gitignored.module.js"
...
var f_execute_query__basic = async function(s_command_suffix){
...
`);
    ...
    var s_command = `mysql --defaults-extra-file=${s_path_file__db_connection_info} ${s_command_suffix}`;
    ...
    await Deno.remove(s_path_file__db_connection_info);
}
var f_execute_query_on_specific_db = async function(s_query, o_database){
    var s_path_file__sqlquery = `./tmp_from_f_execute_query_on_specific_db.sql`;
    await f_write_file(
        s_path_file__sqlquery, 
        s_query);
    await f_execute_query__basic(`${o_database.s_name} < ${s_path_file__sqlquery}`);

    await Deno.remove(s_path_file__sqlquery);
    return Promise.resolve(1);
}
var f_execute_query = async function(s_query){
    return f_execute_query__basic(`-e "${s_query}"`);
}

var f_autogenerate_databases_and_tables = async function(){
    console.log("!! this will drop the database and create it again !!")
    ...
    for(var o_database of a_o_database){
        ...
        await f_execute_query(`CREATE DATABASE IF NOT EXISTS ${o_database.s_name};`)
        // drop it to clear all tables and be sure it does not exist
        await f_execute_query(f_s_query_drop_database(o_database))
        // create it again (with no tables now)
        await f_execute_query(f_s_query_create_database(o_database))

        s_query__use_db = `USE ${o_database.s_name}`;
        s_query += s_query__use_db;

        for(var o_model of a_o_model){
            var s_query_create_table = f_s_query_create_table(o_model, o_db_client);
            s_query += s_query_create_table;
