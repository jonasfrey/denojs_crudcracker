...
var f_s_mysql_type = function(o_model_property){
    ...
    if(
        ...
        s_mysql_type = `VARCHAR(21845)`;//default
        ...
        if(n_maximum_string_length_in_bytes){
            var o__s_mysql_type__n_max_s_len_byte = {
                ...
                [`VARCHAR(${n_maximum_string_length_in_bytes})`]: 21845,//Math.pow(2,16)-1,
...
`;
    ...
    for(var o_model_property__foreign_key of a_o_model_property__foreign_key){
        ...
        if(o_model__foreign && s_name__model_property_foreign){
            ...
            a_s_table_property.push(`${s_i1}${s_i1}FOREIGN KEY (${o_model_property__foreign_key.s_name}) REFERENCES ${f_s_table_name_from_o_model(o_model__foreign)} (${s_name__model_property_foreign})`);
            a_s_table_property.push(`${s_i1}${s_i1}${(s_cascade_on_delete)?'ON DELETE CASCADE': ''}`);
            a_s_table_property.push(`${s_i1}${s_i1}${(s_cascade_on_update)?'ON UPDATE CASCADE': ''}`);
...
var f_execute_query = async function(s_query){
    console.log(`f_execute_query: \`${s_query}\``);
    try{
        await o_db_client.execute(s_query);
    }catch(o){
        console.log(o)
    }
...
USE ${o_database.s_name}`;
    ...
    await f_execute_query(s_query__create_db)
